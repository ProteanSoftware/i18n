...
namespace i18n
{
    ...
    public class TextLocalizer : ITextLocalizer
    {
        ...
        {
            ConcurrentDictionary<string, LanguageTag> AppLanguages = redisContext.Cache.GetObject<ConcurrentDictionary<string, LanguageTag>>($"{environment}.i18n.AppLanguages");
            ...
            lock (Sync)
            {
                AppLanguages = redisContext.Cache.GetObject<ConcurrentDictionary<string, LanguageTag>>($"{environment}.i18n.AppLanguages");
                if (AppLanguages != null)
                {
                    return AppLanguages;
                }
                ...
                redisContext.Cache.SetObject($"{environment}.i18n.AppLanguages", AppLanguages);
        ...
        public virtual string GetText(string msgid, string msgcomment, LanguageItem[] languages, out LanguageTag o_langtag, int maxPasses = -1)
        {
            ...
            DebugHelpers.WriteLine($"{nameof(TextLocalizer)}::{nameof(GetText)} - {msgkey}");
            ...
            if (text != null)
            {
                ...
                DebugHelpers.WriteLine($"{nameof(TextLocalizer)}::{nameof(GetText)} - {msgkey} - {text}");
                if (text == msgkey)
                {
                    return msgid;
                }
